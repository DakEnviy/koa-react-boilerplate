...
['routes', 'store'].forEach((filename) => {
  requireHacker.resolver((pathName, module) => {
    if (pathName === '../../app/' + filename) { // same string as www/all/index
      ...
      })(config.root + '/app/' + filename + '.js'); // needs fullpath
    }
  });
});
